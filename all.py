...
def _handle_transform(data):
    """
    Expert Engine: Replaces DuckDB with Dual-Pass Polars Logic.
    1. Generates Semantic Polars strings via sqlglot.
    2. Executes via SQLContext for live UI preview.
    """
    ...
    if HAS_POLARS:
        local_env.update({'pl': pl, 'col': pl.col, 'lit': pl.lit, 'when': pl.when})

    # 2. Workspace Loading: Inject cached DataFrames into the local namespace
    ...
    try:
        if is_sql and HAS_POLARS:
            # [[ PASS 1: SEMANTIC TRANSPILATION ]]
            # Converts SQL into idiomatic Polars method chains
            try:
                # We use the 'polars' generator to map SQL logic to pl.Expr
                semantic_code = sqlglot.transpile(code, read=None, write="polars")[0]
            except Exception:
                semantic_code = f"# Semantic transpilation unavailable for this syntax"

            # [[ PASS 2: UI PREVIEW EXECUTION ]]
            # Uses Polars native SQL engine for immediate feedback
    ...
    # 4. Table Serialization (High-Contrast Polars View)
    output_text = ""
    if result_obj is not None:
        try:
            if HAS_POLARS and isinstance(result_obj, (pl.DataFrame, pl.Series)):
                ...
                # Aesthetic optimization: wrap Pandas results in Polars for the UI
                output_text = str(pl.from_pandas(pd.DataFrame(result_obj)).head(50))
            else:
                output_text = str(result_obj)
        except Exception:
            output_text = str(result_obj)[:2000]

    return {
        "text_table": output_text,
        "semantic_code": semantic_code  # Passed back to transform.lua for injection
    }

def _handle_metadata_all(data):
    """Metadata extraction for the UI Explorer."""
    cache_dir = data.get("cache_dir", "")
    filter_names = set(data.get("names", []))
    if not os.path.exists(cache_dir): return {"error": "Cache directory missing"}
...
def _handle_preview(data):
    """Plot preview generator for the Viz Wizard."""
    try:
        import matplotlib
        matplotlib.use('Agg')
...
def main():
    """Standard RPC loop reading from stdin and writing to stdout."""
    for line in sys.stdin:
        ...
        try:
            ...
            else: resp = {"error": f"Unknown request: {rtype}"}
